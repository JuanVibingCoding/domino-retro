...
export default function Scoreboard({ players }: { players: { name: string, score: number }[] }) {
  return (
      ...
      <ul>
        {players.map((p, idx) => (
          <li key={idx} className="flex justify-between items-center mb-2">
            <span className="text-handwritten text-xl text-gray-700">{p.name}</span>
            <span className="text-handwritten text-xl text-blue-600">{p.score}</span>
          </li>
        ))}
      </ul>
